...
export default function ClientPage() {
  ...
  async function loadData(userId: string) {
    ...
    const { data: wData } = await supabase.from('workouts').select('*')
      .eq('client_id', userId).eq('week_number', week).eq('day', activeDay).maybeSingle()
    setWorkout(wData)

    const { data: lData } = await supabase.from('client_logs').select('*')
      .eq('client_id', userId).eq('week_number', week).eq('day', activeDay)
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans pb-32">
      ...
      <div className="p-4 space-y-8 max-w-xl mx-auto">
        {loading ? <p className="text-center text-zinc-500 animate-pulse mt-10 font-black uppercase italic">Caricamento...</p> : 
         workout ? ['mobility', 'strength', 'wod'].map((section) => workout[section] && (
          <div key={section} className="bg-zinc-900 rounded-3xl border border-zinc-800 p-6 space-y-4 shadow-2xl">
            ...
            <div className="bg-black/50 border border-zinc-800 p-4 rounded-2xl text-sm text-zinc-200 leading-relaxed whitespace-pre-wrap font-medium">{workout[section]}</div>
            <div className="pt-4 border-t border-zinc-800/50 space-y-4">
               <textarea value={logs[section]?.notes || ''} onChange={(e) => setLogs({...logs, [section]: {...logs[section], notes: e.target.value}})} onBlur={(e) => saveFeedback(section, e.target.value)} placeholder="Note, chili, sensazioni..." className="w-full bg-black border border-zinc-800 p-4 rounded-xl text-xs outline-none focus:border-green-600 min-h-[80px]" />
               <input type="file" accept="video/*" id={`v-${section}`} className="hidden" onChange={(e) => e.target.files?.[0] && handleVideoUpload(section, e.target.files[0])} />
               <label htmlFor={`v-${section}`} className={`w-full flex items-center justify-center gap-2 p-3 rounded-xl border border-zinc-800 text-[10px] font-black uppercase cursor-pointer ${logs[section]?.video_url ? 'bg-green-600/10 border-green-600 text-green-500' : 'bg-zinc-800 text-zinc-400'}`}>
                 {uploadingSection === section ? 'Caricamento...' : logs[section]?.video_url ? '✅ Video Caricato' : '📤 Carica Video'}
               </label>
            </div>
          </div>
        )) : <div className="text-center py-20 bg-zinc-900/50 rounded-3xl border border-dashed border-zinc-800 text-zinc-600 font-black uppercase italic text-sm tracking-widest">Rest Day</div>}
      </div>
    </div>
  )
}