...
export default function TrainerPage() {
  ...
  useEffect(() => {
    if (id) {
      setLogs({}); // SVUOTA TUTTO: Impedisce di vedere i video dell'atleta precedente
      loadWorkout();
      loadLogs();
    }
  }, [id, week, activeDay])
        ...
        {['mobility', 'strength', 'wod'].map((s) => (
          <div key={s} className="bg-zinc-900 rounded-3xl border border-zinc-800 p-6 space-y-4 shadow-2xl">
            ...
            <textarea 
              ...
              placeholder={`Scrivi qui gli esercizi...`} 
            ...
            {logs[s] && (
              <div className="bg-zinc-800/60 border-l-4 border-green-500 p-4 mt-2 rounded-r-2xl space-y-3">
                <p className="text-[10px] font-black text-green-500 uppercase tracking-widest italic">Feedback di questo Atleta</p>
                {logs[s].notes && <p className="text-sm italic text-zinc-300 leading-relaxed">"{logs[s].notes}"</p>}
                {logs[s].video_url && (
                  <div className="rounded-xl overflow-hidden border border-zinc-700 aspect-video bg-black shadow-lg">
